        ...
        sx={[
          (theme) => ({
            ...theme.mixins.bgGradient({
              images: [
                // Gradient dọc (top-bottom)
                `linear-gradient(180deg, ${theme.vars.palette.background.default} 0%, ${varAlpha(
                  theme.vars.palette.background.defaultChannel,
                  alphaLight
                )} 0%, ${theme.vars.palette.background.default} 180%)`,
                // Gradient ngang (trái-phải) để viền ngoài trong hơn
                `linear-gradient(90deg, ${theme.vars.palette.background.default} 0%, ${varAlpha(
                  theme.vars.palette.background.defaultChannel,
                  0
                )} 0%, ${varAlpha(theme.vars.palette.background.defaultChannel, 0)} 150%, ${theme.vars.palette.background.default} 110%)`,
                // Ảnh nền
                `url(${CONFIG.assetsDir}/assets/background/background-vietnam.webp)`,
              ],
            }),
            top: 0,
            left: 0,
            ...
            ...theme.applyStyles('dark', {
              ...theme.mixins.bgGradient({
                images: [
                  ...
                  `linear-gradient(180deg, ${theme.vars.palette.background.default} 12%, ${varAlpha(
                    theme.vars.palette.background.defaultChannel,
                    alphaDark
                  )} 50%, ${theme.vars.palette.background.default} 88%)`,
                  `linear-gradient(90deg, ${theme.vars.palette.background.default} 0%, ${varAlpha(
                    theme.vars.palette.background.defaultChannel,
                    0
                  )} 15%, ${varAlpha(theme.vars.palette.background.defaultChannel, 0)} 85%, ${theme.vars.palette.background.default} 100%)`,
